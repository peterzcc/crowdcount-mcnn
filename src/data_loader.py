...
class ImageDataLoader():
    def __init__(self, data_path, gt_path, mask_path, shuffle=False, gt_downsample=False, pre_load=False):
        ...
        if self.pre_load:
            ...
            if self.gt_downsample:
                wd_1 = int(wd_1/4)
                ht_1 = int(ht_1/4)
                mask = cv2.resize(mask,(wd_1,ht_1),interpolation=cv2.INTER_NEAREST)
                mask = mask
            else:
            ...
            for fname in self.data_files:
                ...
                temp = h5py.File(os.path.join(self.gt_path,os.path.splitext(fname)[0] + '.h5'), 'r')
                den = np.array(temp['density'])
